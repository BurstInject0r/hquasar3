...
function render(title, text, opts) {
  ...
  <div class="tata ${opts.type} ${animate} ${position}" id=${id}>
    ...
    ${!opts.holding && opts.progress ?
      '<div class="tata-progress" id="toastProgressBar"></div>' : ''
    }
  </div>
 `
...
function tataText(title = '你好', text = '今天是' + new Date().toLocaleString(), opts = {}) {
  const _opts = Object.assign({}, defaultOpts, opts, { type: 'text' })
  render(title, text, _opts)
}

function tataLog(title = '你好', text = '今天是' + new Date().toLocaleString(), opts = {}) {
  const _opts = Object.assign({}, defaultOpts, opts, { type: 'log' })
  render(title, text, _opts)
}

function tataInfo(title = '你好', text = '今天是' + new Date().toLocaleString(), opts = {}) {
  const _opts = Object.assign({}, defaultOpts, opts, { type: 'info' })
  render(title, text, _opts)
}

function tataWarn(title = '你好', text = '今天是' + new Date().toLocaleString(), opts = {}) {
  const _opts = Object.assign({}, defaultOpts, opts, { type: 'warn' })
  render(title, text, _opts)
}

function tataError(title = '你好', text = '今天是' + new Date().toLocaleString(), opts = {}) {
  const _opts = Object.assign({}, defaultOpts, opts, { type: 'error' })
  render(title, text, _opts)
}

function tataSuccess(title = '你好', text = '今天是' + new Date().toLocaleString(), opts = {}) {
